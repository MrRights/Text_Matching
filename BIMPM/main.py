...
class Train():
    ...
    def train(self):
        ...
        # 构建字符映射表
        if not os.path.exists(config.char_path):
            build_char(sentences, config.char_path)
        char_to_id = read_char(config.char_path)       # 读取字符表及其映射关系
        # 构建类别映射表
        ...
        # 构建训练数据集
        train_word_data, train_char_data, train_label = data_transform(sentences, labels, word_to_id, char_to_id, label_to_id)
        # 构建验证测试集
        test_word_data, test_char_data, test_label = get_dev_dataset(config.dev_data_path, word_to_id, char_to_id, label_to_id)

        # 打印训练、测试数据量，数据与标签量是否相等
        logging.info('Train Data: {}'.format(np.array(train_word_data).shape))
        logging.info('Train Data: {}'.format(np.array(train_char_data).shape))
        logging.info('Train Label: {}'.format(np.array(train_label).shape))
        logging.info('Test Data: {}'.format(np.array(test_word_data).shape))
        logging.info('Test Data: {}'.format(np.array(test_char_data).shape))
        ...
        for epoch in range(1, config.epochs + 1):
            ...
            batch_train_word_data, batch_train_char_data, batch_train_label = creat_batch_data(train_word_data, train_char_data, train_label, config.batch_size)
            for step, (batch_word_x, batch_char_x, batch_y) in tqdm(enumerate(zip(batch_train_word_data, batch_train_char_data, batch_train_label))):
                feed_dict = {self.model.input_query_word: [x[0] for x in batch_word_x],
                             self.model.input_doc_word: [x[1] for x in batch_word_x],
                             self.model.input_query_char: [x[0] for x in batch_char_x],
                             self.model.input_doc_char: [x[1] for x in batch_char_x],
            ...
            # 将训练数据进行 batch_size 切分
            batch_test_word_data, batch_test_test_data, batch_test_label = creat_batch_data(test_word_data, test_char_data, test_label, config.batch_size)
            for (batch_word_x, batch_char_x, batch_y) in tqdm(zip(batch_test_word_data, batch_test_test_data, batch_test_label)):
                feed_dict = {self.model.input_query_word: [x[0] for x in batch_word_x],
                             self.model.input_doc_word: [x[1] for x in batch_word_x],
                             self.model.input_query_char: [x[0] for x in batch_char_x],
                             self.model.input_doc_char: [x[1] for x in batch_char_x],
...
def get_dev_dataset(data_path, word_to_id, char_to_id, label_to_id):
    '''
    创建验证数据集，并进行预处理
    :param data_path: 测试数据集路径
    :param word_to_id:  word——ID 映射表
    :param char_to_id:  char——ID 映射表
    :param label_to_id: label——ID 映射表
    :param idf: word——IDF 映射表
    :return:
    '''
    logging.info('Get Dev Dataset ...')
    datas_word, datas_char, labels = [], [], []
    for line in tqdm(open(data_path, mode='r', encoding='UTF-8')):
        line_list = line.strip().replace('\n', '').split('\t')
        if line_list[2] == 'label':     # 跳过头标签
            continue

        # 将文本word转换为ID表示[1, 6, 2, 3, 5, 8, 9, 4]
        sentence_word_id_temp = []
        for sentence in line_list[: 2]:
            sentence_word_temp = []
            for word in text_processing(sentence).split(' '):
                if word in word_to_id:  # 如果词汇在词表中，则进行ID表示
                    sentence_word_temp.append(word_to_id[word])
                else:   # 如词汇不在词表中，则表示为OOV词汇ID
                    sentence_word_temp.append(word_to_id['<UNK>'])

            # 对文本长度进行padding填充
            sentence_length = len(sentence_word_temp)
            if sentence_length > config.seq_length:     # 对超长文本进行截断
                sentence_word_temp = sentence_word_temp[: config.seq_length]
            else:           # 对长度不足文本进行padding
                sentence_word_temp.extend([word_to_id['<PAD>']] * (config.seq_length - sentence_length))
            sentence_word_id_temp.append(sentence_word_temp)
        datas_word.append(sentence_word_id_temp)

        # 将文本char转换为ID表示[1, 6, 2, 3, 5, 8, 9, 4]
        sentence_char_id_temp = []
        for sentence in line_list[: 2]:
            sentence_char_temp = []
            for char in list(''.join(text_processing(sentence).split(' '))):
                if char in char_to_id:  # 如果字符在词表中，则进行ID表示
                    sentence_char_temp.append(char_to_id[char])
                else:  # 如字符不在词表中，则表示为OOV字符ID
                    sentence_char_temp.append(char_to_id['<UNK>'])

            # 对文本长度进行padding填充
            sentence_length = len(sentence_char_temp)
            if sentence_length > config.seq_length:  # 对超长文本进行截断
                sentence_char_temp = sentence_char_temp[: config.seq_length]
            else:   # 对长度不足文本进行padding
                sentence_char_temp.extend([char_to_id['<PAD>']] * (config.seq_length - sentence_length))
            sentence_char_id_temp.append(sentence_char_temp)
        datas_char.append(sentence_char_id_temp)
    ...
    return datas_word, datas_char, labels
...
def build_char(input_data, char_path):
    '''
    根据数据集构建字符表，存储到本地备用
    :param input_data: 全部句子集合 [n, 2] n为数据条数
    :param char_path: 字符文件存储路径
    :return:
    '''
    logging.info('Build Char ...')
    all_sentence = []   # 全部句子集合
    for sentence_list in tqdm(input_data):
        for sentence in sentence_list:
            all_sentence.extend(list(''.join(sentence)))

    counter = Counter(all_sentence)     # 字频统计
    count_pairs = counter.most_common(config.char_size - 2)       # 对字符按次数进行降序排序
    chars, _ = list(zip(*count_pairs))      # 将(char, count)元祖形式解压，转换为列表list
    # 添加一个 <PAD> 来将所有文本pad为同一长度
    chars = ['<UNK>'] + list(chars)     # 增加一个OOV标识的编码
    chars = ['<PAD>'] + list(chars)     # 增加一个PAD标识的编码
    open(char_path, mode='w', encoding='UTF-8').write('\n'.join(chars))
...
def read_char(char_path):
    ...
    :return: 字符表，char_to_id
    """
    chars = [char.replace('\n', '').strip() for char in open(char_path, encoding='UTF-8')]
...
def data_transform(input_data, input_label, word_to_id, char_to_id, label_to_id):
    '''
    数据预处理，将文本和标签映射为ID形式，额外特征计算IDF
    :param input_data: 文本数据集合
    :param input_label: 标签集合
    :param word_to_id: 词汇——ID映射表
    :param char_to_id: 字符——ID映射表
    ...
    for sentence_list in tqdm(input_data):
        ...
        for sentence in sentence_list:
            ...
            if sentence_length > config.seq_length:
                ...
                sentence_word_temp.extend([word_to_id['<PAD>']] * (config.seq_length - sentence_length))
            sentence_word_id_temp.append(sentence_word_temp)
        sentence_word_id.append(sentence_word_id_temp)
    ...
    for sentence_list in tqdm(input_data):
        ...
        for sentence in sentence_list:
            sentence_char_temp = []
            for char in list(''.join(sentence)):
            ...
            if sentence_length > config.seq_length:
                ...
                sentence_char_temp.extend([char_to_id['<PAD>']] * (config.seq_length - sentence_length))
            sentence_char_id_temp.append(sentence_char_temp)
        sentence_char_id.append(sentence_char_id_temp)
    ...
    # shuffle
    indices = np.random.permutation(np.arange(len(sentence_word_id)))
    datas_word = np.array(sentence_word_id)[indices]
    datas_char = np.array(sentence_char_id)[indices]
    labels = np.array(label_id)[indices]

    return datas_word, datas_char, labels


def creat_batch_data(input_word_data, input_char_data, input_label, batch_size):
    '''
    将数据集以batch_size大小进行切分
    :param input_word_data: word数据列表
    :param input_char_data: char数据列表
    :param input_label: 标签列表
    :param input_feature: 额外特征列表
    :param batch_size: 批大小
    :return:
    '''
    max_length = len(input_word_data)            # 数据量
    max_index = max_length // batch_size    # 最大批次
    # shuffle
    indices = np.random.permutation(np.arange(max_length))
    data_word_shuffle = np.array(input_word_data)[indices]
    data_char_shuffle = np.array(input_char_data)[indices]
    ...
    for index in range(max_index):
        ...
        batch_word_data.append(data_word_shuffle[start: end])
        batch_char_data.append(data_word_shuffle[start: end])
        batch_label.append(label_shuffle[start: end])

        if (index + 1) * batch_size > max_length:           # 如果结束索引超过了数据量，则结束
            break

    return batch_word_data, batch_char_data, batch_label
