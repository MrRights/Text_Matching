...
class Model():
    def __init__(self):
        ...
        # 对所有词向量进行叠加求平均，用来表征整个文本向量
        embedding_query = tf.reduce_mean(embedding_query, axis=1)
        embedding_pos_doc = tf.reduce_mean(embedding_pos_doc, axis=1)
        embedding_neg_doc = tf.reduce_mean(embedding_neg_doc, axis=2)

        # 全连接层layer1 (batch_size, 300) -> (batch_size, 300)
        L1_N = 300
        l1_range = tf.sqrt(6/(self.config.embedding_dim + L1_N))        # 原论文weight、bias范围初始化方式
        weight_1 = tf.get_variable(initializer=tf.random_uniform(shape=[self.config.embedding_dim, L1_N], minval=-l1_range, maxval=l1_range), name='weight-1')
        bias_1 = tf.get_variable(initializer=tf.random_uniform(shape=[L1_N], minval=-l1_range, maxval=l1_range), name='bias-1')
        # 全连接
        query_l1 = tf.matmul(embedding_query, weight_1) + bias_1
        pos_doc_l1 = tf.matmul(embedding_pos_doc, weight_1, ) + bias_1
        neg_doc_l1 = tf.matmul(tf.reshape(embedding_neg_doc, shape=[-1, self.config.embedding_dim]), weight_1, ) + bias_1
