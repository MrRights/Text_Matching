...
class Train():
    ...
    def train(self):
        ...
        # 构建训练数据集
        train_data = data_transform(sentences, word_to_id)
        # 构建验证测试集
        ...
        # 模型训练
        best_acc_score = 0    # 初始best模型的accuracy值
        for epoch in range(1, config.epochs + 1):
            train_accuracy_list = []  # 存储每个epoch的accuracy
            train_loss_list = []  # 存储每个epoch的loss
            # 将训练数据进行 batch_size 切分
            batch_train_data = creat_batch_data(train_data, config.batch_size)
            for step, batch_x in tqdm(enumerate(batch_train_data)):
                ...
                train_accuracy, train_loss, _ = self.sess.run([self.model.accuracy_train, self.model.loss, self.model.optimizer], feed_dict=feed_dict)
            ...
            # 模型验证
            test_correct_count = 0      # 统计正确结果数
            for (test_x, test_y) in tqdm(zip(test_data, test_label)):
                feed_dict = {self.model.input_query: [test_x[0]],
                             self.model.input_pos_doc: [test_x[1]],
                             self.model.input_keep_prob: 1.0}
                query_doc_cosine = self.sess.run(self.model.query_pos_cosine, feed_dict=feed_dict)[0][0]
                if round(query_doc_cosine) == int(test_y):          # cosine_score 四舍五入，是否与true label相等
                    test_correct_count += 1
            acc_val_final = test_correct_count*1.0/len(test_data)   # 计算正确率

            logging.info('Test Epoch: %d , Acc: %.6f' % (epoch, acc_val_final))

            # 当前epoch产生的模型acc值超过最好指标时，保存当前模型
            if best_acc_score < acc_val_final:
                best_acc_score = acc_val_final
...
class Predict():
    ...
    def pre_process(self, sentence):
        ...
        # 将词汇映射为ID
        sentence_id = [0] * config.vocab_size  # 每个文本表示为词表大小的向量
        for word in sentence_seg:
            if word in self.word_to_id.keys():  # 如果当前词汇在词表中，则词汇频次+1
                sentence_id[self.word_to_id[word]] += 1
            else:  # 如果当前词汇不在词表中，则UNK频次+1
                sentence_id[self.word_to_id['<UNK>']] += 1
...
def get_dev_dataset(data_path, word_to_id):
    ...
    for line in tqdm(open(data_path, mode='r', encoding='UTF-8')):
        ...
        for sentence in sentence_list[: 2]:
            sentence_temp = [0] * config.vocab_size
            for word in text_processing(sentence).split(' '):
                if word in word_to_id.keys():       # 如果当前词汇在词表中，则词汇频次+1
                    sentence_temp[word_to_id[word]] += 1
                else:       # 如果当前词汇不在词表中，则UNK频次+1
                    sentence_temp[word_to_id['<UNK>']] += 1
...
def data_transform(input_data, word_to_id):
    ...
    for sentence_list in tqdm(input_data):
        sentence_id_temp = []
        for sentence in sentence_list:
            sentence_temp = [0]*config.vocab_size       # 每个文本表示为词表大小的向量
...
def creat_batch_data(input_data, batch_size):
    '''
